...
SOURCE_BRANCH="${SOURCE_BRANCH:-offload-oc}"
...
cherry_pick_personal_commit() {
	...
	printf 'Replaying personal commit %s: %s\n' \
		"${commit}" "$(git show -s --format=%s "${commit}")"
	if ! git cherry-pick --empty=keep "${commit}"; then
		git status --short >&2
...
replayed_personal_count="$(git rev-list --count "${new_base_sha}..HEAD")"
[ "${replayed_personal_count}" -eq "${#personal_commits[@]}" ] || die \
	"composition did not preserve every personal commit"
